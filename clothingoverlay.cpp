...
PNG clothingoverlay::renderImage(){
    ...
    int xStart = (baseWidth - itemWidth) / 2;
    ...
    for (int i = 0; i < itemWidth; i++){
        for (int j = 0; j < itemHeight; j++){
            ...
            loc.first = i + xStart;
            loc.second = j;
            changeColor(baseImage, itemImage, loc, xStart);
        }
    }
...
void clothingoverlay::changeColor(PNG & baseIm, PNG & itemIm, pair<int,int> loc, int xStart){
    ...
    // get itemIm pixel
    int itemX = x - xStart;
    RGBAPixel* itemPixel = itemIm.getPixel (itemX,y);
