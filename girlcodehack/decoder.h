...
public:
    ...
    // uses last coord in pathPts
    pair<int,int> findSpot();

    // returns the location of the treasure
    // uses shortest paths table d
    pair<int,int> findSpot(vector<vector<int>> & d);
...
private:
    ...
    // changes the pixel at position loc to red (255,0,0)
    void setRed(PNG & im, pair<int,int> loc);
